...
Baloon::Baloon(float x, float y, float z) {
    this->position = glm::vec3(x, y, z);
    this->velocity = glm::vec3(0,0, -10);
    this->acc = glm::vec3(0,0,-10);
    this->rotation = 0;
...
void Baloon::draw(glm::mat4 VP,float x,float y,float z) {
...
void Baloon::set_position(float x, float y,float z) {
    this->position = glm::vec3(x, y, z);
}

//x is forward coming out of screen
void Baloon::tick(int up) {
    //this->rotation += speed;
...
bounding_box_t Baloon::bounding_box() {
